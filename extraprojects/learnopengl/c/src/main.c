...
key_callback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
    ...
    if (key == GLFW_KEY_F1 && action == GLFW_PRESS){
        ...
        worldgenChunk(&world, chunk, currentChunkX, currentChunkZ);
        point2d chunkPos = {currentChunkX, currentChunkZ};
        hmput(world.chunkMap, chunkPos, chunk);
        ++currentChunkX;
        if (currentChunkX % 20 == 0){
            ++currentChunkZ;
            currentChunkX = 0;
        }
    }
...
main()
{
    ...
    while (!(glfwWindowShouldClose(window))) {
        ...
        for (int k = 0; k < hmlen(world.chunkMap); ++k){
            vec2 camdirvec = {player_camera.dir.x, player_camera.dir.z};
            vec2 camposvec = {player_camera.pos.x, player_camera.pos.z};
            vec2 chunkposvec = {world.chunkMap[k].pos.x * WORLDGEN_CHUNK_LEN, world.chunkMap[k].pos.z * WORLDGEN_CHUNK_LEN};

            i32 mychunkX = (i32)player_camera.pos.x / WORLDGEN_CHUNK_LEN;
            i32 mychunkZ = (i32)player_camera.pos.z / WORLDGEN_CHUNK_LEN;
            //printf("mychunk: %d %d\n", mychunkX, mychunkZ);
            ...
            //printf("angle between %f %f and %f %f: %f\n",
            //        camdirvec[0], camdirvec[1],
            //        chunkdirvec[0], chunkdirvec[1],
            //        dotangle);
            //usleep(1000000);
            float anglediff = fabs(180.0 - dotangle);
            if (anglediff > player_camera.zoom 
                    && !(mychunkX == world.chunkMap[k].pos.x && mychunkZ == world.chunkMap[k].pos.z)) {
                //printf("skipping %d %d\n", world.chunkMap[k].pos.x, world.chunkMap[k].pos.z);
                continue;
            }
            else
                ++chunksrendered;
            for (int i = 0; i < WORLDGEN_CHUNK_LEN; ++i){
                for (int j = 0; j < WORLDGEN_CHUNK_LEN; ++j){
                    block b = world.chunkMap[k].chunk->blocks[i][j];
                    ...
                    // Draw the right stuff for this particular block
                    glBindVertexArray(blockVAOs[b.type]);
